...
const ManageUsers = () => {
    ...
    useEffect(() => {
        fetch(`https://smart-seba-server.onrender.com/users/`)
            .then(res => res.json())
            .then(data => {
                setReFetch(false)
    ...
    const handleAddMoney = async (email) => {
        ...
        fetch(`https://smart-seba-server.onrender.com/users/bikash`, {
            method: "PATCH",
            headers: {
    ...
    const handleDeleteUser = (email) => {
       ...
       if(agreed){
        fetch(`https://smart-seba-server.onrender.com/users/${email}`, {
            method: "DELETE",
            headers: {
                'Content-Type': 'application/json'
            },
        ...
        <div className='overflow-x-scroll block'>
            ...
            <div className="overflow-x-scroll">
                <table className="table block table-xs table-pin-rows table-pin-cols overflow-x-scroll">
                    ...
                    <tbody className=''>
                        {
                            usersData?.map(data => <tr key={data._id}>
                                ...
                                <td className='text-[15px] flex flex-row items-center justify-start gap-2'>
                                    <label className="form-control ">
                                        ...
                                        <input onChange={(e) => setAmount(e.target.value)} type="number" required placeholder="amount" name='number' className="input input-bordered w-[100px]" />

                                    </label>
                                    <button className='btn btn-sm' onClick={() => handleAddMoney(data?.email)}>Add Money</button>
                                </td>
