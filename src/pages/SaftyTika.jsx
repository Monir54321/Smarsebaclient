                    ...
                    fetch(`https://smart-seba-server.onrender.com/users/${user?.email}`)
                        ...
                        .then(data => {
                            if (data?.data?.amount >= price) {
                                ...
                                // post data to database
                                fetch("https://smart-seba-server.onrender.com/saftyTikas/", {
                                    method: "POST",
                                    headers: {
        ...
        <div className='w-full p-10 min-h-screen '>
            <form onSubmit={handleSubmit} className='flex flex-col items-center'>
                ...
                <label className="form-control w-full ">
                    ...
                    <input type="text" placeholder=" পাসপোর্ট নাম্বার" name='passportNumber' className="input input-bordered w-full" />
                ...
                <label className="form-control w-full ">
                    ...
                    <input type="text" placeholder="(OTP) ফোন নাম্বার:" name='otpPhoneNumber' className="input input-bordered w-full" />
                            ...
                            <tbody>
                                {
                                    myOrders?.map(data => <tr key={data._id}>
                                        ...
                                        <td className='text-[15px] flex flex-row w-full'>
                                            {
                                                data?.status == "Success" && <button onClick={() => alert("You can't delete until successfully delivering this service")}>
                                                    <MdDelete className='w-5 h-5 font-semibold text-blue-600' width={16} height={16} />
                                                </button>
                                            }

                                            {
                                                data?.title == "Success" && <button>
